(async () => {
  ...
  require('dotenv').config(
    MIGRATION_TEST ? { path: path.resolve(process.cwd(), '.env.test') } : {}
  );
    ...
    if (actions.indexOf(args[0]) <= -1) {
      logger.error(
        'First argument must be one of: ' +
          ...
          '. The default value is "automatic-up".'
      );
      return false;
    }
  ...
  let migration = new migrationClass(
    MIGRATION_TEST
      ? MIGRATION_CONFIG_TEST_VAR_FILENAME
      : MIGRATION_CONFIG_VAR_FILENAME
  );
  ...
  if (action === 'automatic-up') {
    if (await migration.executeAutomaticUp()) {
      logger.warning(
        'Automatic up migration has ended. Review var/migration.json'
      );
