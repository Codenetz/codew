let
  clientLanguage = require("../lib/clientLanguage"),
...
module.exports = async (req, res, next) => {
  ...
  if(ClientLanguage.isLanguageForChange()) {
    ...
    let
      ...
      /** Gets the wanted language by query parameter */
      ...
      hostname = redirect_url_parts.hostname.replace(ClientLanguage.getSubdomain() + ".", ""),

      /** Sets the new subdomain if the language is not default */
      subdomain = (default_language.domain === language.domain ? "" : language.domain + ".");
  ...
  if(ClientLanguage.isOnDifferentLanguageSubdomain()) {
    ...
    let
      language = ClientLanguage.findLanguageByCookie(),
  ...
  if(ClientLanguage.isNewClientOnRootDomain()) {
    ...
    let
      language = await ClientLanguage.findLanguageByGeoLocation(),
  ...
  if(ClientLanguage.isMissingSubdomainWithCookie()) {
    ...
    let
      language = ClientLanguage.findLanguageByCookie(),
      redirect_to = "//" + language.domain + "." + req.hostname + req.originalUrl;
  ...
  if(ClientLanguage.isOnDefaultSubdomain()) {
    let redirect_to = "//" + req.hostname.replace(ClientLanguage.getDefaultLanguage().domain + ".", "") + req.originalUrl;
    ClientLanguage.changeLanguage(ClientLanguage.getDefaultLanguage(), redirect_to);
    return;
  }
